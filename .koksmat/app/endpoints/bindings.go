...
func AddEndpoints(s *web.Service, jwtAuth func(http.Handler) http.Handler) {
	s.Route("/v1", func(r chi.Router) {
		r.Group(func(r chi.Router) {
			...
			r.Method(http.MethodPost, "/test/basic", nethttp.NewHandler(TestBasicPost()))
			r.Method(http.MethodPost, "/test/sharepoint", nethttp.NewHandler(TestSharepointPost()))
			r.Method(http.MethodPost, "/test/exchange", nethttp.NewHandler(TestExchangePost()))
